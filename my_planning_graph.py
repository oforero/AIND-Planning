...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        def link_action_node(a_node, s_node):
            s_node.children.add(a_node)
            a_node.parents.add(s_node)
            return a_node

        a_nodes = [PgNode_a(a) for a in self.all_actions]
        self.a_levels.append({link_action_node(a_node, s_node)
    ...
    def add_literal_level(self, level):
        ...
        def update(a_node, s_node):
            a_node.children.add(s_node)
            s_node.parents.add(a_node)
            return s_node

        s_nodes = {update(a_node, s_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (a1_add & a2_rem) or (a2_add & a1_rem)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        a1_add, a1_rem = set(node_a1.action.effect_add), set(node_a1.action.effect_rem)
        a1_pos, a1_neg = set(node_a1.action.precond_pos), set(node_a1.action.precond_neg)
        a2_add, a2_rem = set(node_a2.action.effect_add), set(node_a2.action.effect_rem)
        a2_pos, a2_neg = set(node_a2.action.precond_pos), set(node_a2.action.precond_neg)

        return (a1_add & a2_neg) or (a1_rem & a2_pos) or (a2_add & a1_neg) or (a2_rem & a1_pos)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        pre_a1 = node_a1.parents
        pre_a2 = node_a2.parents

        return any(pre1.is_mutex(pre2) for pre1 in pre_a1 for pre2 in pre_a2)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        return all(act1.is_mutex(act2) for act1 in actions_1 for act2 in actions_2)
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        def min_level_for_goal(goal):
            return next(i
                        for i, level in enumerate(self.s_levels)
                        for node_s in level
                        if node_s.is_pos and node_s.symbol == goal)

        # implement for each goal in the problem, determine the level cost, then add them together
        return sum([min_level_for_goal(goal) for goal in self.problem.goal])
